...
suite('TranslationSet', () => {
    const addTranslationSets = async function (): Promise<TranslationSets> {
        ...
        settings.translationFiles['en'] = vscode.workspace.asRelativePath(enFile);
        settings.translationFiles['de'] = vscode.workspace.asRelativePath(deFile);
    ...
    test('translation set contains translations', async () => {
        ...
        expect(translationSet.keys.length).to.eq(5);
